...
import getOpportunityLineItems from '@salesforce/apex/OpportunityLineItemController.getQuoteLineItems';
import updateOpportunityLineItems from '@salesforce/apex/OpportunityLineItemController.updateQuoteLineItems';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import { refreshApex } from '@salesforce/apex';
import { getPicklistValues, getObjectInfo } from 'lightning/uiObjectInfoApi';
import OPPORTUNITY_LINE_ITEM_OBJECT from '@salesforce/schema/QuoteLineItem';
import MARGIN_TYPE_FIELD from '@salesforce/schema/QuoteLineItem.Margin_Type__c';
import COMMISSION_TYPE_FIELD from '@salesforce/schema/QuoteLineItem.Commission_Type__c';
import PACKAGING_FIELD from '@salesforce/schema/QuoteLineItem.Packing__c';
...
import sendPriceStatusEmails from '@salesforce/apex/QuoteStatusEmailHelper.sendQuoteStatusEmails';
...
export default class OpportunityLineItemsLWC extends NavigationMixin(LightningElement) {
    ...
    @wire(getOpportunityLineItems, { QuoteId: '$recordId' })
    ...
    wiredPakagingTypeValues({ error, data }) {
        if (data) {
            ...
            console.error('Error fetching Packing__c picklist values:', error);
        }
    }
    ...
    wiredLineItemsResult({ error, data }) {
        if (data) {
            ...
            this.lineItems = data.map(item => {
                ...
                const incoTerms = item.Quote.Inco_Terms__c || 'undefined';
    ...
    handleChange(event) {
        ...
        if (this.lineItems[index].Commission_Type__c === 'Percent') {
            ...
            commissionAmount = (commissionPercent / 100) * unitPrice;
    ...
    handleSave() {
        ...
        updateOpportunityLineItems({ lineItems: this.lineItems })
            .then(() => {
                ...
                // Get Quote Ids from lineItems
                const quoteIds = this.lineItems.map(item => item.QuoteId); // Ensure you are mapping the correct field for QuoteId

                // Send email notifications based on updated status
                sendPriceStatusEmails({ quoteIds: quoteIds })
                    .then(() => {
