...
import createSampleRequest from '@salesforce/apex/SampleRequestController.createSampleRequest';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import getUserOptions from '@salesforce/apex/SampleRequestController.getUserOptions';
import getSamplesRequestSentToOptions from '@salesforce/apex/SampleRequestController.getSamplesRequestSentToOptions';
import getProductOptions from '@salesforce/apex/SampleRequestController.getProductOptions';
import getOpportunityById from '@salesforce/apex/SampleRequestController.getOppcountryById';
import getaccountdetails from '@salesforce/apex/SampleRequestController.getopptails';
import getOpportunityAccountAndCountry from '@salesforce/apex/SampleRequestController.getOpportunityAccountAndCountry';
import { NavigationMixin } from 'lightning/navigation';
import getCurrentUserName from '@salesforce/apex/MarketingForm.getCurrentUserName';
import getUnitOfMeasureOptions from '@salesforce/apex/SampleRequestController.getUnitOfMeasureOptions';
...
export default class LwcSampleRequest extends NavigationMixin(LightningElement) {
    ...
    opportunityData({ error, data }) {
        if (data) {
            ...
            this.accountId = data.AccountId;  // Set Company Id from Opportunity
            this.companyName = data.Account.Name;  // Set Company Name from Opportunity
            this.countryId = data.Account.Country__c;  // Set Country Id from Account
            this.countryName = data.Account.Country__r.Name;  // Set Country Name from Account
        } else if (error) {
    ...
    handleFieldChange(event) {
        ...
        if (fieldName === 'sampleOutSentTo') {
            ...
            console.log('Company Name changed to:', this.accountId);
    ...
    handleCancel() {
        // Navigate back to the Opportunity record page
        this[NavigationMixin.Navigate]({
            type: 'standard__recordPage',
            attributes: {
                recordId: this.recordId, // Use the recordId of the current Opportunity
                objectApiName: 'Opportunity', // Object API name for Opportunity
    ...
    handleSubmitSampleIn() {
        ...
        const productData = this.products.map(product => ({
            ...
            productName: product.productName,  // Assuming productName contains Product2 Id
            quantity: product.quantity,
            quality: product.quality,
            colourIndex: product.colourIndex,
            application: product.application,
