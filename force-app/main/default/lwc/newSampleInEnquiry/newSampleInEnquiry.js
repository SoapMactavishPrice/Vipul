...
import createSampleInRequest from '@salesforce/apex/SampleInRequestEnquiry.createSampleInRequest';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import getUserOptions from '@salesforce/apex/SampleInRequestEnquiry.getUserOptions';
import getProductOptions from '@salesforce/apex/SampleInRequestEnquiry.getProductOptions';
import getOpportunityById from '@salesforce/apex/SampleInRequestEnquiry.getOppcountryById';
import getaccountdetails from '@salesforce/apex/SampleInRequestEnquiry.getopptails';
import getOpportunityAccountAndCountry from '@salesforce/apex/SampleInRequestEnquiry.getOpportunityAccountAndCountry';
import { NavigationMixin } from 'lightning/navigation';
import getCurrentUserName from '@salesforce/apex/MarketingForm.getCurrentUserName';
import getUnitOfMeasureOptions from '@salesforce/apex/SampleInRequestEnquiry.getUnitOfMeasureOptions';
import fetchCI from '@salesforce/apex/MarketingForm.fetchCI';

export default class NewSampleInEnquiry extends NavigationMixin(LightningElement) {
    ...
    handleUnitSelected(event) {
        try {
            ...
            console.log(`✅ Unit selected for row ${index}`, unitId, unitName);
        } catch (error) {
    ...
    handleUnitCleared(event) {
        ...
        console.log(`❌ Unit cleared for row ${index}`);
    }
    ...
    connectedCallback() {
        ...
        this.receivedDate = today1.toISOString().split('T')[0]; // Format: YYYY-MM-DD
    }
    ...
    handleSubmitSampleIn() {
        ...
        createSampleInRequest(requestData)
            .then(result => {
                console.log('Sample In Request created successfully:', result);
                this[NavigationMixin.Navigate]({
                    type: 'standard__recordPage',
                    attributes: {
                        recordId: result,
                        objectApiName: 'Sample_In_Request__c',
                        actionName: 'view'
                    }
                });
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Success',
                        message: 'Sample In Request created successfully!',
                        variant: 'success'
                    })
                );
            ...
            .catch(error => {
                console.error('Error creating Sample In Request:', error);
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error creating Sample In Request',
